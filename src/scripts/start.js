...
gltfLoader.load(
    ...
    (gltf) => {
        ...
        gltf.scene.traverse((child) => {
            if (child.isMesh) {
                // Apply original material to all meshes initially

                // Check and apply specific material for the 'String' mesh
                if (child.name === 'model001') {
                    child.material = bakedMaterial ; // Override material for the 'String' mesh
                }
            }
        });
...
renderer.setClearColor(0x000000, 1); // Set the background color to #FEFEFA
